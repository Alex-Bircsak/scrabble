...
$(function() {
  ...
  // Do a jQuery Ajax request for the text dictionary
    ...
    //$.get( "/usr/share/dict/words", function( txt ) {
  ...
  // Initialize the pool of tiles with all duplicates included
  ...
  // Add tiles to the rack and update the remaining tiles
    function initializeRackTiles(){
      ...
      $("#tiles-remaining").text("Remaining Tiles in Pool: " + tilePool.length + "/100") //update remaining tile html
    }

  // Track the total score
    function calculateScore() {
      console.log("Calculating score...");
      console.log("Dictionary size = " + dict.length + " words");
  ...
  // Update score when a tile is added to the board
  ...
  // Handle all interactions when a tile is dropped onto the tileboard, utilizing helper functions to properly mantain score, current word, remaining tiles
  ...
  // Update the current word as the user plays
    function updateWord() {
      var currentWord = "";
      //update the current word by parsing board tiles
      $("#tileBoard div").each(function(index,$el) {
  ...
  // Handle only adding tiles required for full hand of 7 after word is submitted
    function fillRackForNextHand() {
      ...
      if (currentRack.length < 7) {  //check if rack isnt completely empty for case where a word was played successfully
        ...
        $("#tiles-remaining").text("Tiles Remaining: " + tilePool.length + "/100") //update tiles remaining
  ...
  // Create tile html elements and append them to tile rack
    ...
    // Set tiles to be draggable, modify css, and configure draggable settings.
      // Learned about this from: https://api.jqueryui.com/draggable/#option-snapMode
  ...
  // Create dialog when a blank tile is placed on the board
